...
La stessa molecola di topoisomerasi trova delle SAR disponibili o delle SAR incapaci.
Lo stesso vale per le molecole di topoisomerasi adiacenti. Evidentemente è la stessa topoisomerasi: perché dovrebbero essere più o meno affini molecole dello stesso enzima? Ci deve essere un qualche segnale chimico che rende le molecole di topoisomerasi in quella regione più affini rispetto a tutte le altre molecole di topoisomerasi.
Tutti questi sono segnali epigenetici perché la sequenza del DNA è sempre la stessa, non cambia. 
Cosa succede? Intervengono delle piccole molecole che sono in grado di modificare localmente la struttura del DNA (in questo caso della SAR) rendendola aperta o chiusa, oppure che modificano la proteina rendendola più o meno affine alle altre proteine della sua stessa categoria. 
Queste piccole molecole oggi sappiamo per certo essere fondamentalmente RNA non tradotti, ormai i maggiori protagonisti di tutti gli eventi di regolazione epigenetica dell’espressione genica.
La chiave dell’epigenetica è che la stessa sequenza all’occorrenza è aperta o chiusa. Questo è il primissimo livello al quale iniziamo a vedere questo tipo di regolazione fondamentale. Perché? Perché è evidente che, se è vero che un loop è un’unità funzionale allora, se io voglio avere una regione che è molto attiva trascrizionalmente, devo avere dei loop piccoli (in cui ci sia fondamentalmente un solo gene che viene continuamente trascritto) e molto lassi.
    • In una zona trascrizionalmente molto attiva avremo loop piccoli e lassi
In una zona trascrizionalmente poco attiva o comunque meno trascritta la situazione è invece diversa: distinguiamo tra geni housekeeping e geni tessuto-specifici. I geni housekeeping sono quelli del metabolismo di base che in tutte le cellule sono trascritti lentamente ad un livello basale; i geni tessuto-specifici sono quelli che in quel particolare tessuto devono essere trascritti ai massimi livelli.
Cellule diverse, in due tessuti diversi dello stesso organismo, hanno delle esigenze tissutali trascrizionali diverse: i geni tessuto-specifici saranno in loop piccoli e molto lassi. Quella stessa regione nella cellula di un altro tessuto sarà ipercondensata.
    • I geni housekeeping, in generale, si trovano in loop piuttosto grandi, abbastanza condensati perché devono essere trascritti in maniera basale in tutte le cellule di tutti gli organismi
    • Se abbiamo una regione completamente inerte: loop molto grandi e molto molto condensati. In questo caso parliamo di eterocromatina costitutiva.
      
    • Nei loop piccoli c’è dentro un solo gene e per questo sono trascrizionalmente attivi.
    • Nei loop grandi invece ci sono dentro più unità trascrizionali che vengono trascritte in sequenza e quindi ci vuole più tempo. 
Tieni sempre a mente che, durante la trascrizione e la replicazione, la doppia elica del DNA si deve aprire, i nucleosomi si staccano in maniera transiente per poi riattaccarsi subito dopo perché il DNA deve essere accessibile al complesso trascrizionale o al complesso replicativo. Affinchè questo avvenga, la cromatina deve essere in una conformazione adatta: si devono rapidamente staccare gli 8 istoni che formano il nucleosoma ma, appena completato il processo di trascrizione e di replicazione, gli istoni dell’ottamero si riassociano al DNA e tutto questo è modulato dalla struttura ad anse.
...
La regolazione a livello trascrizionale è quella fondamentale perché è quella più economica. Ci sono tanti altri livelli di regolazione dell’espressione genica perché regolare l’espressione genica vuol dire arrivare alla proteina matura funzionante: la regolazione che permette di fare la maggiore economia è quella già a livello di trascrizione perché questo evita sprechi metabolici. 
...
Le modificazioni epigenetiche non riguardano promotori, enhancers e silencers di per sé, ma piuttosto l’accessibilità di tali sequenze ai fattori con cui interagiscono (mano e guanto). Le modificazioni epigenetiche fanno in modo che le sequenze regolatrici siano o meno capaci di interagire con i propri effettori.
Le modificazioni consistono fondamentalmente in:
    • Metilazione del DNA.
    • Metilazione ed acetilazione di specifici gruppi degli istoni che formano l’ottamero.
Una cosa fondamentale è che in ogni regione del genoma degli eucarioti superiori esiste un rapporto ben preciso tra grado di metilazione e grado di acetilazione e addirittura questo rapporto locale tra numero di acetili e numero di metili rappresenta una sorta di codice istonico che si sovrappone al codice genetico, proprio perché ha la capacità di modulare l’espressione genica localmente. In qualche caso il codice istonico è stato decodificato: ci sono delle regioni del genoma in cui si è stabilito esattamente qual è il rapporto chiave tra metili e acetili e quindi il codice istonico per avere un certo livello di espressione genica). 
    • Il codice istonico è il rapporto locale in una regione genomica tra numero di metili e numero di acetili. Questo rapporto ha un valore fondamentale per stabilire il livello di espressione genica in quella particolare regione del genoma.
Le SAR sono state facilmente isolate e caratterizzate. Trattiamo il DNA con la stessa sostanza usata per ottenere il back bone proteico (litio 3’, 5 diiodiosalcilato, LIS): rimuovo gli istoni. Rimuovendo gli istoni, ci saranno delle regioni legate alla topoisomerasi che sono protette perché schermate quando faccio una digestione con enzimi di restrizione. Posso quindi rimuovere la topoisomerasi e caratterizzare quei segmenti di DNA che erano stati protetti dalla digestione enzimatica. In questo modo sono state sequenziate le SAR.
Tutti questi esperimenti sono stati fatti fondamentalmente in Drosophila melanogaster.
Posso anche misurare la dimensione dei segmenti che si trovano tra due regioni protette dalla digestione enzimatica e quindi valutare la dimensione dei loops e cioè la distanza tra SAR adiacenti. Nella Drosophila è molto variabile la dimensione dei loops: da 4,5 a 115 Kb.


Tutte le SAR studiate contengono da 8 a 17 box ricorrenti. Per box si intende un blocco di sequenza: blocchi ben specifici, molto simili tra loro in SAR diverse, ricorrenti e cioè che si ritrovano identici o molto simili in tutte le SAR.
In tutte le SAR è presente un sito di legame per la topoisomerasi ed è costituito dalla sequenza GNT(A/T)A(T/C)ATTNATNN(G/A). 
Ci sono due box ricorrenti tra quelle trovate confrontando le diverse SAR della Drosophila melanogaster. Queste due box hanno un significato regolativo: sono presenti o assenti a seconda che ci troviamo in una regione di geni housekeeping (che non devono essere regolati perché son trascritti in tutte le cellule di tutto l’organismo) o di geni tessuto-specifici (regolati). Abbiamo quindi una T-box che è una sequenza ricca in T ed una A-box che è ricca in A. La T-box si trova a valle della A-box. La differenza tra geni housekeeping e geni  tessuto-specifici sta proprio in queste box regolative.
