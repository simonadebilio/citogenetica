...
\section{La FISH a due colori}
La FISH a due colori è una tecnica piuttosto banale. Voglio identificare due sonde diverse che sono generalmente molto vicine sul cromosoma: voglio capire quanto sono distanti, se sono sovrapposte o se sono riarrangiate nel caso di una particolare patologia.
Devo usare metodi di marcatura diversi per poi poter evidenziare le sonde in maniera diversa.
In realtà sono tre i colori che mi servono. Ho due sonde che devo identificare sui cromosomi. I cromosomi sono sempre controcolorati in blu (almeno nella stragrande maggioranza dei casi) perché si usa il DAPI che è un colorante che colora in blu i cromosomi, ma in più consente di ottenere un bandeggio. Quindi i cromosomi non sono solo colorati, ma sono anche identificabili con un bandeggio: il bandeggi DAPI è sovrapponibile ad un bandeggio di tipo Q che quindi mi permette di identificare il cromosoma bersaglio.
La controcolorazione è sempre blu: devo quindi avere comunque un microscopio dotato di tre filtri. La sonda 1 la marco ad esempio con biotina; la sonda 2 la marco ad esempio con digossigenina. 
Poi evidenzierò la sonda 1 per esempio con avidina coniugata con un fluorocromo rosso; la sonda 2 con un anticorpo anti-digossigenina per esempio coniugato con la fluoresceina.
Quindi avrò i cromosomi pseudocolorati di blu (pseudocolorati perché le immagini sono acquisite con i toni di  grigio). Sui cromosomi controcolorati in blu, vedrò rossa la sonda 1 e verde la sonda 2.
Devo quindi avere sistemi diversi di marcatura perché questo mi permette di avere sistemi diversi di evidenziazione.

FISH A PIÙ COLORI 
Qui si salta da 2 colori a 7 colori: i cromosomi sono sempre controcolorati in blu, ma uso 3 sistemi di marcatura (prima ne avevo usati due: digossigenina e biotina-avidina). I 3 sistemi di marcatura sono: biotina, digossigenina e il terzo metodo è una marcatura diretta con la fluoresceina quindi uso UTP già coniugato con il fluorocromo.
La FISH a più colori può essere fatta in maniera “casalinga”, senza avere dei sistemi di marcatura commerciali (ma poi se si arriva a n marcature ovviamente le miscele non le posso fare in casa) e non devo neanche avere un software dedicato.
La FISH a più colori si può fare con un qualunque microscopio.
Marco:
    • La sonda 1 con biotina.
    • La sonda 2 con digossigenina.
    • La sonda 3 direttamente con fluoresceina.

Evidenzio:
    • La biotina con avidina coniugata con Ultralitre che emette nell’ultravioletto.
    • La sonda 2 con digossigenina coniugata con rodamina che emette nel rosso.
Quindi ho bisogno di un filtro per l’Ultravioletto, di un filtro per il rosso e di un filtro per il verde (che mi serve per quello che ho marcato con la fluoresceina). In più ovviamente il filtro per il blu per vedere i cromosomi controcolorati.
Quindi ho un sistema di 3 marcature e 3 filtri. Ma come faccio a vedere contemporaneamente 7 sonde?
È banale. Un’altra sonda la marco con una miscela 1:1 (50 e 50) di biotina e digossigenina. Questa sonda io la vedrò sia con il filtro da 680 nm, sia con il filtro da 630 nm, ma è l’unica che vedo con questa coppia di filtri.
Quindi:
    • La sonda 1 è l’unica che vedo esclusivamente con il filtro a 680 nm.
    • La sonda 2 è l’unica che vedo solo con il filtro a 630 nm.
    • La sonda 3 è l’unica che vedo con un solo filtro.
    • La sonda 4 è l’unica che vedo con l’accoppiamento di filtri: sia con il 680 sia con il 630.
    • La sonda 5 (biotina e fluoresceina) è l’unica che vedo con il 680, ma non con il 630, bensì con il 580.
    • La sonda 6 (dove ho messo digossigenina e fluoresceina) è l’unica che vedo con il 630 e con il 580.
    • L’ultima sonda la marco con una miscela 1:1:1 di UTP coniugato con ciascuno dei sistemi di marcatura e quindi è l’unica che vedo con tutti e tre i filtri.
...
Allora cosa faccio?
Pseudocoloro del colore 1 tutte le immagini che vedo solo con il primo filtro, del colore 2 quelle che vedo soltanto con il secondo, del colore 3 quelle che vedo soltanto il terzo filtro, del colore 4 quelle che vedo soltanto con una coppia di filtri, del colore 5 per un’altra coppia di filtri, colore 6 per l’ultima coppia di filtri e infine colore 7 per il terzetto di filtri.
E questo naturalmente lo posso fare in maniera casalinga con 3 filtri (anzi, 4 perché mi serve quello per il DAPI).
Come avviene l’osservazione con più filtri?
Guardo il preparato. Uso il primo filtro: con il primo filtro vedo alcuni segnali, acquisisco l’immagine e assegno lo pseudocolore 1. Guardo lo stesso preparato ma uso il secondo filtro: con il secondo filtro vedo alcuni segnali che erano quelli di prima, ma anche altri segnali, acquisisco l’immagine e assegno lo pseudocolore 2. Così anche per il 3 filtro e assegno lo pseudocolore 3. Poi sarà fondamentale fare una sovrapposizione delle immagini acquisite con i 3 filtri diversi. L’immagine è sempre la stessa identica ed è fondamentale quindi avere dei sistemi che mi permettano di cambiare filtri senza che si sposti minimamente il preparato: se l’immagine si sposta anche solo di pochissimo, poi la sovrapposizione non avviene correttamente.
Ogni immagine con un singolo filtro viene acquisita con un tono di grigio e sovrapposta. Alla fine ho le singole immagini a cui ho assegnato un diverso pseudocolore. Ottimizzo le immagini e poi il software fa la sovrapposizione.
Quindi ho queste 7 sonde su un singolo cromosoma: saprò in che sequenza sono, a che distanza sono e potrò avere anche moltissime altre informazioni in proposito.
È chiaro che è complicato e macchinoso se non ho un software che mi fa tutto in maniera automatica, ma si può comunque fare in maniera casalinga: lo possiamo fare con un software che permette di fare solo l’analisi, la pseudocolorazione, il merge. Questi stessi software diventeranno indispensabili quando aumentano ulteriormente i colori e costano moltissimo (20-30mila euro).
È fondamentale aver capito il principio perché il poter passare dai 7 colori ai 24 e più colori non è altro che l’estensione dello stesso identico principio. I metodi di marcatura di solito non sono più di 4, ma le miscele sono in rapporti diversi: per esempio, anziché 1:1 e quindi 50 e 50, io posso fare 1:3 e quindi cambia il rapporto di toni di grigio. È ovvio che io non lo posso apprezzare, ci vuole un software che assegna uno pseudocolore alla differenza di rapporto di toni di grigio tra i fluorocromi che ho utilizzato.
Quindi in realtà, anche con pochi sistemi di marcatura, se faccio miscele diverse con rapporti diversi (anziché 50 e 50 posso fare 1:3 o 1:4 ecc), posso aumentare enormemente il numero di colori senza avere mille sistemi di marcatura. C’è un limite ai sistemi di marcatura che posso utilizzare come c’è un limite per i fluorocromi, ma non c’è un limite al numero di pseudocolori perché me li invento. Gli pseudocolori dipendono dal rapporto di intensità di grigio tra un fluorocromo e l’altro.
...
RIASSUMENDO.
    • Le immagini ottenute alle diverse lunghezze d’onda vengono acquisite indipendentemente in bianco e nero e archiviate.
    • Le singole immagini vengono elaborate per ottimizzare i parametri di acquisizione per ogni lunghezza d’onda.
    • Poi viene attribuito un diverso pseudocolore ad ogni sonda.
    • Un software dedicato consente poi di fare il merge e cioè di sovrapporre le singole immagini pseudocolorate e ottenere un’immagine dove i cromosomi sono blu e poi ci sono sopra colori diversi.

M-FISH o SKY FISH
Lo stesso principio viene usato per fare quelle che vengono chiamate M-FISH (Multicolor FISH) o Sky FISH che permettono di fare i cosiddetti “spectral karyotype”. Siccome sono quelle che permettono di colorare tutti i cromosomi di colori diversi, si usano delle painting probes.
...
Anche qui la misurazione della variazione di rapporti di toni di grigio e la pseudocolorazione sono fatte da un software dedicato, non riesco a farle in maniera artigianale: fino a 7 colori riesco, ma di più diventa veramente impossibile. Poi pseudocoloro ogni cromosoma a seconda dei rapporti di intensità di toni di grigio dei diversi sistemi di marcatura (le marcature le vedo con diversi filtri, non è il microscopio che deve essere diverso).
...
Mi devo procurare dei kit di marcatura per i singoli cromosomi e un software che mi permetta di fare l’analisi e la pseudocolorazione dei diversi rapporti di toni di grigio tra 5 fluorocromi utilizzati contemporaneamente (non tanto il merge perché quello lo fanno tutti i software).
Come si fa? Fondamentalmente per ogni cromosoma c’è l’eccitazione con una diversa lunghezza d’onda o con diverse combinazioni di lunghezze d’onda. Siccome per ogni cromosoma c’è una diversa combinazione, per ogni cromosoma poi c’è uno pseudocolore diverso. Quello che ottengo sono immagini molto belle con i cromosomi tutti di colori diversi. 
Questo approccio viene usato normalmente per l’analisi dei cariotipi molto riarrangiati nelle cellule tumorali, oltre che per l’identificazione di marcatori cromosomici. Ad uno stadio avanzato della progressione del tumore ci sono moltissimi riarrangiamenti e inoltre i cariotipi dei tumori solidi in stadio avanzato sono veramente un puzzle di segmenti cromosomici e, guardando l’immagine sotto, è palese che varia il numero cromosomico. 
...
I tumori solidi sono differenti rispetto alle leucemie. Nelle leucemie faccio l’analisi del midollo quindi riesco ad analizzare le cellule primordiali tumorali e quindi riesco ad analizzare i primi riarrangiamenti. I tumori solidi si analizzano a partire da un prelievo bioptico del tumore, quando già è in uno stadio abbastanza avanzato e quindi quando già ci sono tanti riarrangiamenti.
L’analisi citogenetica dei tumori consente di capire, identificando i punti di rottura, quali sono i geni coinvolti nello sviluppo, nell’eziologia, causativi del cancro, ma consente anche di capire quali sono i geni coinvolti nel processi di progressione e metastatizzazione. Quindi l’approccio citogenetico è fondamentale per la comprensione dei meccanismi di sviluppo e di evoluzione dei tumori. 
Prima dell’avvento di queste tecniche, la citogenetica dei tumori solidi diceva pochissimo perché erano informazioni frammentarie. Al contrario si sapeva di tutto e di più sulla citogenetica delle leucemie e dei linfomi proprio perchè è molto più semplice.
Questi approcci hanno permesso di fare della citogenetica seria. È evidente che, se in un tumore ho dei riarrangiamenti coì complessi, capire qual è quello causativo e quello secondario non è banale. Ma, dal momento che oggi abbiamo delle banche dati che raccolgono le informazioni da tutti i laboratori del mondo e riportano tutti i riarrangiamenti coinvolti nello stesso tipo di tumori, ecco allora che ci si avvicina all’identificare (e in qualche caso è stato fatto) il riarrangiamento che rappresenta il minimo comune denominatore di tutti i tumori della stessa classe. Per fare questo ovviamente bisogna avere un gran numero di dati: tutti questi dati non si avevano finché non ci sono stati questi approcci che hanno permesso di fare della seria citogenetica dei tumori solidi.
Il primo approccio per identificare l’oncogene coinvolto in un particolare cancro è proprio l’approccio citogenetico.



COMPARATIVE GENOMIC HYBRIDIZATION
L’ultima parte che riguarda l’ibridazione in situ è un altro approccio che è stato anch’esso una conquista straordinaria nel campo diagnostico, non soltanto per il cancro, ma anche per l’identificazione di riarrangiamenti criptici in patologie costituzionali (come la Sky FISH).
Questo approccio si chiama Comparative Genomic Hybridization (CGH). Il nome è eloquente: ibridazione genomica comparata. 
La CGH è straordinaria perché permette di fare delle analisi citogenetiche (cioè identificare riarrangiamenti cromosomici) senza bisogno di avere il cariotipo delle cellule in cui si vuole scoprire il riarrangiamento.
Prendiamo l’esempio del cancro. Supponiamo di avere un prelievo bioptico maltrattato. Normalmente le cellule del tumore solido crescono come delle pazze in vivo; nel momento in cui io le metto in coltura non crescono. Non solo, spesso sono tessuti necrotici.
Per fare della citogenetica (e quindi vedere i cromosomi), le cellule si devono ben dividere. Se non si dividono è complicatissimo: un altro limite infatti della citogenetica dei tumori solidi è il fatto che normalmente ho del materiale che non è adatto per fare le analisi. Il materiale è adatto se riesco a fare delle linee cellulari, ma, nel momento in cui io impianto una linea cellulare, questa si è già adattata alle condizioni in vitro e quindi resta un punto interrogativo su quanto questa sarà simile al tumore di partenza.
Con la CGH io posso fare l’analisi cromosomica dei riarrangiamenti presenti nel cancro anche avendo un prelievo necrotico e cellule che non si replicano. Perché? Perché lo faccio semplicemente utilizzando il DNA. Il DNA è resistente: anche se ho un frammentino necrotico, posso fare comunque l’estrazione del DNA e la purificazione; il DNA se mai è un pochino frammentato ma non è neanche detto.
Non posso fare la coltura ai cromosomi perché non si replicano, ma l’estrazione del DNA la posso fare, non c’è problema. A partire semplicemente dal DNA della cellula in cui devo identificare i riarrangiamenti, io posso fare della citogenetica di quella cellula, senza avere i suoi cromosomi. È una tecnica molto importante.
Il limite è che la CGH, al contrario della Sky FISH, non va bene per i riarrangiamenti bilanciati (traslocazioni o inversioni, scambi), ma va bene per i riarrangiamenti sbilanciati e cioè tutti i riarrangiamenti che comportano la perdita o l’acquisto di regioni genomiche (delezioni e duplicazioni) anche in assenza di divisione cellulare. 
...
È particolarmente utile per l’analisi di materiale qualitativamente scarso o per campioni di tessuto molto poveri o per cellule che non si replicano. Ideale per l’analisi di riarrangiamenti complessi, purchè prevedano alterazioni del dosaggio.
Come funziona?
Il DNA genomico viene estratto da cellule che si suppone abbiano riarrangiamenti, come  nel nostro esempio la biopsia del cancro. Omogenizzo, estraggo il DNA, lo purifico, faccio l’estrazione di tutte le proteine. Alla fine ottengo in una provetta il campione del DNA genomico della cellula da testare che chiamiamo DNA test (DNA genomico della cellula tumorale, purificato). Marco il DNA ad esempio con biotina.
Poi prendo del DNA genomico da cellule di controllo di un individuo sano (di solito si estrae dal sangue di un donatore volontario). 
Ho quindi il DNA genomico di controllo e il DNA genomico del tumore. Li marco in maniera differente: il DNA di controllo posso marcarlo ad esempio con digossigenina.
Dopodichè faccio una miscela 50-50 del DNA del tumore e del DNA di controllo che sono marcati in modo diverso e li ibrido su cromosomi normali di un controllo (prendo un volontario e faccio dei preparati da sangue periferico). 
L’ibridazione quindi la faccio su cromosomi che so essere perfettamente normali: la miscela 1 a 1 delle due sonde si ibrida con i cromosomi normali e poi evidenzio le due differenti sonde, una in verde e una in rosso.
Cosa mi aspetto di vedere? Tutti i cromosomi saranno gialli (perché verde + rosso fa giallo): c’è un rapporto 1 a 1 tra fluorescenza verde e fluorescenza rossa e quindi tutti i cromosomi saranno gialli.
MA se nel cancro c’era una duplicazione in una certa regione, allora per quella regione, non ci sarà un rapporto 1 a 1 tra quello che è marcato di verde e quello che è marcato di rosso: se c’è una duplicazione, c’è un eccesso di rosso (se era marcato in rosso il DNA). 
In quella regione dove nel cancro c’era una duplicazione, sui miei cromosomi non avrò giallo, ma avrò un eccesso di rosso e quindi lo vedrò come una bandina di colore diverso.
Se viceversa c’era una delezione avrò un eccesso di verde e difetto di rosso e quindi lo vedrò come un rapporto di fluorescenza diverso rispetto all’atteso che è pari 1.
In alcuni casi questo è eclatante e lo si vede ad occhio nudo, ma comunque marco i cromosomi: io mi aspetto una fluorescence ratio (rapporto di fluorescenza) verde/rosso pari a 1. Un software mi fa la scansione e mi calcola il rapporto di fluorescenza punto per punto su tutti i cromosomi: mi aspetto di avere rapporti di fluorescenza poco variabili rispetto ad una retta che rappresenta l’1.
Quando ho dei picchi in su o dei picchi in giù, quella è una regione dove c’è eccesso di verde o eccesso di rosso (a seconda del metodo di marcatura) e quindi delezione o duplicazione nel tumore. 
Ecco che quindi è fondamentale che il bersaglio siano cromosomi normali: è solo sui cromosomi normali che riesco a vedere se c’è duplicazione o delezione. Se i cromosomi sono del tumore, nel punto in cui c’è la delezione ci sarà comunque il rapporto 1 a 1.
...
Il DNA test viene evidenziato in verde mentre il DNA di controllo in rosso (esempio).
Un software dedicato, scansionando tutto il genoma, calcola per ogni cromosoma e per ogni regione il rapporto di fluorescenza rosso/verde. Se nel DNA test un cromosoma o un frammento cromosomico è deleto, avrò in quella regione un eccesso di fluorescenza rossa rispetto a quella verde. Viceversa, se nel DNA test un cromosoma o un frammento è duplicato, avrò in quella regione un eccesso di fluorescenza verde rispetto a quella rossa. 
Le differenze sono difficilmente apprezzabili ad occhio nudo, ma il software mi fa vedere dove ci sono dei picchi.
Questo approccio è fondamentale. In questo periodo in cui è protagonista la genomica e la post-genomica, protagonista è anche la citogenomica che è una citogenetica fatta con approcci di post-genomica. 
Questa tecnica, così com’è, viene fatta utilizzando dei microchip. Tenendo presente lo stesso principio di prima, anziché avere come bersaglio della miscela 1 a 1 delle sonde il cromosoma, ho come bersaglio un microchip.
...
Quindi ho un vetrino su cui sono microspottati dei BAC che coprono quella particolare regione del cromosoma: gli spot sono dei BAC quindi aumento molto la risoluzione che è ora delle centinaia di Kilobasi.
Il bersaglio non è il cromosoma citogeneticamente definito e quindi con i limiti della risoluzione citogenetica. 
Allora cosa faccio?
L’esperimento è identico.  
I microchip vengono fatti da delle company: io dico qual è il tratto di genoma che voglio che venga spottato e le company mi preparano il microchip. Dopodichè si fa l’ibridazione con un rapporto 1 a 1. 
Poi ci sono dei sistemi di analisi automatizzati che fanno la scansione del vetrino e mi segnalano gli spot con eccesso di rosso o eccesso di verde. 
Quindi è Comparative Genomic Hybridization con una risoluzione superiore perché usa degli approcci di post-genomica.
Questa tecnica è fondamentale perché mi consente di identificare non tanto i riarrangiamenti dei tumori, ma posso identificare quelle che si chiamano copy number variation che sono fondamentali in molte patologie.
Quindi: stessa identica tecnica, ma risoluzione superiore perché uso dei microchip come bersaglio anziché dei preparati cromosomici. La risoluzione diventa molecolare anziché citogenetica.
Le applicazioni sono fondamentalmente per l’analisi dei tumori, ma anche per l’identificazione di marcatori cromosomici e l’identificazione di particolari aneuploidie in aborti spontanei. La tecnica è utile dovunque si sospetti che ci siano riarrangiamenti dovuti a perdita o acquisto di materiale genetico.
Non permette di identificare riarrangiamenti bilanciati per i quali bisogna invece ricorrere alla Sky-FISH. 
